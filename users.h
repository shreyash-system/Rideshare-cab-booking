...
public:
    ...
    void displayProfile() {
        ...
        cout << "Emergency Contact: " << (emergencyContact.empty() ? "Not set" : emergencyContact) << endl;
        cout << "Emergency Phone: " << (emergencyPhone.empty() ? "Not set" : emergencyPhone) << endl;
...
public:
    ...
    static UserAccount* login() {
        ...
        bool found = db->getUserByUsername(username, fullName, phone, emergencyContact, emergencyPhone, paymentMethod);
        ...
        cout << "\nUsername not found. Would you like to create a new account? (Y/N): ";
        char choice;
        cin >> choice;
        if (choice == 'Y' || choice == 'y') {
    ...
    static void setEmergencyContact(UserAccount* user) {
        ...
        while (user->emergencyPhone.length() != 10 || !isValidPhone(user->emergencyPhone)) {
            cout << "Invalid phone. Enter 10 digits: ";
            getline(cin, user->emergencyPhone);
        }

        // Update in database
        if (db->updateUserEmergencyContact(user->username, user->emergencyContact, user->emergencyPhone)) {
    ...
    static void setPaymentMethod(UserAccount* user) {
        ...
        switch (choice) {
            case 1: user->paymentMethod = "Cash"; break;
            case 2: user->paymentMethod = "Card"; break;
            case 3: user->paymentMethod = "UPI"; break;
            case 4: user->paymentMethod = "Wallet"; break;
            default: user->paymentMethod = "Cash";
        }
