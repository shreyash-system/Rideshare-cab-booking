...
int main() {
    try {
        ...
        while (true) {
            ...
            cout << "  Logged in as: " << currentUser->fullName << " (" << currentUser->phone << ")" << endl;
            UI::printSpacer();
            ...
            if (choice == 1) {
                ...
                if (cType == "mini") cType = "Mini";
                else if (cType == "prime") cType = "Prime";
                else if (cType == "suv") cType = "SUV";
                else {
                ...
                if (distance > 0) {
                    cout << "  [OK] Distance calculated: " << fixed << setprecision(2) << distance << " km" << endl;
                } else {
                    cout << "  [WARNING] Could not calculate distance automatically." << endl;
                ...
                if (drivers.empty()) {
                    UI::printError("No drivers available! Please register drivers first.");
                    UI::waitForEnter();
                    continue;
                }
                ...
                string cabType;
                if (cabChoice == 1) { selectedCab = new Mini(); cabType = "Mini"; }
                else if (cabChoice == 2) { selectedCab = new Prime(); cabType = "Prime"; }
                else { selectedCab = new SUV(); cabType = "SUV"; }
                ...
                if (rideID > 0) {
                    ...
                    if (!scheduledDate.empty()) {
                        cout << "  Scheduled For: " << scheduledDate << " at " << scheduledTime << endl;
                        UI::printSpacer();
                    }
                } else {
                ...
                vector<tuple<int, string, string, string, string, int, double>> activeRides = db.getActiveRides();
                if (activeRides.empty()) {
                    ...
                    if (distance > 0) {
                        cout << "  [OK] Distance: " << fixed << setprecision(2) << distance << " km" << endl;
                    } else {
                        cout << "  Enter distance manually (in km): ";
                        cin >> distance;
                    }
                    ...
                    if (!drivers.empty()) {
                        ...
                        int rideID = db.insertRide(
                            currentUser->fullName,
                            currentUser->phone,
                            pickup, drop, cabType, distance, fare,
                            assignedDriver.getName(),
                            assignedDriver.getVehicleNum(),
                            1, "Active", currentUser->paymentMethod, "", ""
                        );
                ...
                UI::printSpacer();
                cout << "  ID\tPickup\t\tDrop\t\tType\tPass.\tFare/Person" << endl;
                cout << "  ==================================================================" << endl;
                for (size_t i = 0; i < activeRides.size(); i++) {
                    ...
                    cout << "  " << rideID << "\t" << pickup.substr(0, 12) << "\t" 
                         << drop.substr(0, 12) << "\t" << cabType << "\t" << passengers 
                         << "\t" << fixed << setprecision(2) << (fare / passengers) << endl;
                }
                ...
                if (db.addPassengerToRide(rideID, currentUser->fullName)) {
                    double farePerPerson = db.getFarePerPerson(rideID);
                    UI::printSuccess("Ride shared successfully!");
                    cout << "  Your fare: Rs." << fixed << setprecision(2) << farePerPerson << endl;
                } else {
                    UI::printError("Failed to join ride! It might be full or not found.");
                }
                ...
                vector<tuple<int, string, string, string, double, int, double, string>> rideHistory = db.getRidesByPhone(currentUser->phone);
                ...
                for (size_t i = 0; i < rideHistory.size(); i++) {
                    ...
                    if (status == "Active") {
                        hasActiveRides = true;
                        cout << "  " << rideID << "\t" << pickup.substr(0, 12) << "\t" 
                             << drop.substr(0, 12) << "\t" << status << "\t\t" 
                             << fixed << setprecision(2) << fare << endl;
                    }
                }
                ...
                if (activeRides.empty()) {
                    ...
                    cout << "\n  ID\tPickup\t\tDrop\t\tType\tPass.\tFare/Person" << endl;
                    cout << "  ==================================================================" << endl;
                    for (size_t i = 0; i < activeRides.size(); i++) {
                        int rideID = get<0>(activeRides[i]);
                        string pickup = get<1>(activeRides[i]);
                ...
                if (rideHistory.empty()) {
                    ...
                    for (size_t i = 0; i < rideHistory.size(); i++) {
                        ...
                        UI::printRideCard(rideID, pickup, drop, cabType, passengers, totalFare);
                ...
                if (db.updateUserEmergencyContact(currentUser->username, emergencyContact, emergencyPhone)) {
                ...
                if (currentUser->emergencyContact.empty()) {
                    UI::printError("No emergency contact set! Please set one first (Option 8).");
                } else {
                    UI::printSpacer();
                ...
                cout << "\n  Current payment method: " << currentUser->paymentMethod << endl;
                UI::printSpacer();
                ...
                switch (payChoice) {
                    ...
                    case 4: paymentMethod = "Wallet"; break;
                    default: 
